...
// HashingVault is a highly customizable hash table
// that uses separate chaining instead of using linked lists for the chains,
// some general containers are used
// a container can be a binary search tree or a smart array
// or actually any class that implements the Container class
...
public:
    ...
    // and return true if the key is not already there
    // use the hash function to find which slot to add the item
    // add to the Container at that slot
    // (i.e. table[x] where x is obtained
    // by applying the hash function to the given key)
    // with Container's add function
    ...
    // if key is not found, this function would return
    // whatever value returned by the Container's get function
    // for the key-not-found situation
    ValueType get(KeyType key) const;
    
    // if the specified key exists,
    // remove the item that has it and return true
    ...
    // print the hash table, showing all items in each of the slots,
    // to the given ostream given; do not modify
