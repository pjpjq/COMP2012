...
private:
#include "bstNode.h" // since we only use BSTNode in BST class,
    // we include it here under "private"
    
    BSTNode<KeyType, ValueType> *root = nullptr; // the root node;
    // empty tree's root is nullptr
    int size = 0; // the number of nodes in the tree including the root node;
    // it will not be checked when we grade your work,
    // but correctly maintaining this number may be useful
    // for your implementation in certain member functions
